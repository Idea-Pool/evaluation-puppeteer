describe("TC-5 Interaction with radio form elements", () => {
    ...
    test("The default radio should be enabled", async () => {
        const isDisabled = await page.$eval("#exampleRadios1", box => box.disabled);
        expect(isDisabled).toBe(false);
    });
    test("The disabled radio should be disabled", async () => {
        const isDisabled = await page.$eval("#exampleRadios3", box => box.disabled);
        expect(isDisabled).toBe(true);
    });
    test("The default radio should be selected", async () => {
        const isSelected = await page.$eval("#exampleRadios1", box => box.checked);
        expect(isSelected).toBe(true);
    });
    test("The second default radio should not be selected", async () => {
        const isSelected = await page.$eval("#exampleRadios2", box => box.checked);
